...
from user import User, UserCitizenship, get_current_user, oauth2_scheme
...
async def upload_file(file: UploadFile, phone: str, 
...
async def get_file(
    ...
    u = session.exec(
        select(Citizenship)
        .where(Citizenship.verified == False)).first()
...
async def verify_identity( phone:str, user: UserCitizenship,
