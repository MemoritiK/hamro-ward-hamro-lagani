...
async def get_file(
    ...
    u = session.exec(select(Citizenship).where(Citizenship.status == "pending")).first()
    if u is None:
        raise HTTPException(status_code=404, detail="No user found.")
    
    user = session.exec(select(User).where(User.phone == u.phone)).first()
